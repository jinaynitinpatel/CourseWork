...
mpcObj.Weights.OV = [1000,0;1000 1000];
...
% Plant equations: output update
    ...
    %options.OutputWeights = [1000,0;1000,100];
    ...
    y1_diff = sum(abs(ympc(1:end,1) - r(1)*ones(length(T), size(length(T),1))));
    y2_diff = sum(abs(ympc(1:end,2) - r(2)*ones(length(T), size(length(T),1))));
    u1_diff = sum(umpc(1:end,1) - u(1)*ones(length(T), size(length(T),1)));
    u2_diff = sum(umpc(1:end,2) - u(2)*ones(length(T), size(length(T),1)));
    w1 + w2 + w3 + w4 = 1;
    P = w1*y1_diff + w2*y2_diff + w3*u1_diff + w4*u2_diff;
...
subplot(4,2,1)
plot(T, ympc(:,1), 'b', T, (r_p-r_tot)*cos(phi_array), ':r') 
title('Position');
ylabel('X (in m)');
xlabel('Elapsed Time (seconds)')
%legend('Tracking Trajectory', 'Reference Trajectory');
subplot(4,2,2)
plot(T, ympc(:,2), 'b', T, (r_p-r_tot)*sin(phi_array), ':r')
title('Position');
ylabel('Y (in m)');
xlabel('Elapsed Time (seconds)')
%legend('Tracking Trajectory', 'Reference Trajectory');
...
plot(ympc(:,1),ympc(:,2),'b', x_los_1, y_los_1, 'r', x_los_2, y_los_2, 'r', x_los_3, y_los_3, 'g')
title('x-y plot');
ylabel('y (in m)');
xlabel('x (in m)')
